...
範疇論中有一個共通的結構，稱為*通用結構*(universal construction)，用於根據物件之間的關係來定義物件。一種方法是選一個模式，由物體和態射構成的特殊外觀，並在範疇中找尋所有它發生的地方。如果這是一個普遍的模式且範疇夠大的話，那發生機會會大大增加。這個訣竅是在這些匹配中建立某種排名，然後選擇最適合的匹配。
<!--stackedit_data:
eyJoaXN0b3J5IjpbMjAxMDMwNzY4OCwyMDk5NjQ1MzQwXX0=
-->