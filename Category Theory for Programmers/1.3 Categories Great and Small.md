...
您可能會問這個問題，是否每個範疇么半群 — 一個物件的範疇 — 定義一個唯一的二元運算子集合的么半群。事實證明我們總是可以單從單一物件範疇提取一個集合。這集合是態射集合 — 在我們例子中的加法器。換句話說，在範疇M中我們有單一物件m的hom-set   ![formula](https://render.githubusercontent.com/render/math?math=Hom(m,m)) 。我們可以輕鬆地在此集合中定義一個二元運算子：么半積 (monoidal product)
<!--stackedit_data:
eyJoaXN0b3J5IjpbMTQ1NzY3MjYzOCwyODQwMTcwNjUsMTQ2OD
