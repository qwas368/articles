...
例如，這裡有一個加 5 的操作，可以對每個自然數加5。它映射 0 到 5、1 到 6、2 到 7，依此類推。這函數定義於自然數集合之上。這很好：我們有一個函數和一個集合。通常來ㄕㄨㄛ，對於任何數字 n 都有一個加 n 函數 — n 的加法器。

這個加法器怎麼做到組合？加5的函數與加7的函數組合成加12的函數。因此加法器的組合可以等同加法規則，
<!--stackedit_data:
eyJoaXN0b3J5IjpbMTU4MDU5MDg3NCwxMDU2NzAxMTQ4LC0xND
