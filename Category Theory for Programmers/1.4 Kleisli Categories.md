...
我在本文中所用來作為範疇基礎的特定monad稱為`writer` monad，它用於log或追蹤函數的執行情況。這也是在純計算中嵌入影響的更通用機制的一個範例。你之前已經看過，我們可以在集合範疇中建立模程式語言型別的模型和函數（通常不考慮bottom）。
<!--stackedit_data:
eyJoaXN0b3J5IjpbLTE1MDIzMDM5MDIsLTEwNjM4Njc2NjgsLT
