...
你可能已經猜到我並沒有當場發明這個類別。這是所謂的Kleisli範疇(Kleisli Category)的例子 — 一種基於單子(monad)的型別。我們尚未準備好討論monad，但我想讓你先了解它們的功能。出於有限的目的，Kleisli範疇有底層程式語言的型別作為物件。從型別A到型別B的態射是使用從A到用特定修飾派生的B型別的函數。每個Kleisli範疇都定義了自己組合態射的方式，以及關於該組合的恆等態射。（稍後，我們會看到不精確的術語“裝飾”對應於範疇中的自函子(Endofunctor)的概念。）

我在本文中用作類別基礎的特定monad稱為writer monad，它用於記錄或跟踪功能的執行情況。
<!--stackedit_data:
eyJoaXN0b3J5IjpbMTEwNTAyMzY2MCwtMTA2Mzg2NzY2OCwtNj
