...
精明的讀者可能會注意到，很容易將這種結構推廣到任何么半群 (monoid)，而不僅僅是字串么半群。我們在 `compose` 中使用 `mappend` 並在 `identity` 中使用`mempty`（代替`+`和 `""`）。 確實沒有理由將自己限制為僅log成字串。一個好的編寫函式庫供ㄥ應該能夠找出使圖書館工作的最小限度的限制
<!--stackedit_data:
eyJoaXN0b3J5IjpbMTgxNTk3NjMyMSwzNjgzNTYyNjgsMTk5Nz
MyMzI1NCw4MDgxMjYwNjAsLTE0MDg0ODUzNDgsLTE0NTUwMzc4
NTQsLTE5MjkwOTMwMiwyNTA4NDkyNTQsLTgxOTM5NDI5NF19
-->