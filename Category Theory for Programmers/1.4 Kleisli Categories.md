...
精明的讀者可能會注意到，很容易將這種結構推廣到任何么半群 (monoid)，而不僅僅是字串么半群。我們在 `compose` 中使用 `mappend` 並在 `identity` 中使用`mempty`（代替`+`和 `""`）。 確實沒有理由將自己限制為僅log成字串。一個好的編寫函式庫工程式應該能夠找出使函式庫運作的最小限度限制。在此，log 函式庫的唯一要求是Log必須具有么半屬性。
<!--stackedit_data:
eyJoaXN0b3J5IjpbLTEwMjQ4MTc5NTEsMzY4MzU2MjY4LDE5OT
czMjMyNTQsODA4MTI2MDYwLC0xNDA4NDg1MzQ4LC0xNDU1MDM3
ODU0LC0xOTI5MDkzMDIsMjUwODQ5MjU0LC04MTkzOTQyOTRdfQ
==
-->