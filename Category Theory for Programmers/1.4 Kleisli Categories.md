...
我在本文中所用來作為範疇基礎的特定monad稱為`writer` monad，它用於log或追蹤函數的執行情況。這也是在純計算中嵌入影響的更通用機制的一個範例。你之前已經看過，我們可以在集合範疇中建立模程式語言型別的模型和函數（通常不考慮bottom）。在這裡，我們已將此模型擴展到一個稍有不同的範疇，該範疇的態射由修飾函數表示，它們的組合不僅僅將一個函數的輸出傳遞給另一個函數的輸入。我們還有一個更大的自由度：組合本身。事實證明，這正是自由度，這使得有可能為傳統上使用副作用實現的命令式語言賦予簡單的指稱語義。
<!--stackedit_data:
eyJoaXN0b3J5IjpbLTE3NTA1MDY1NjQsLTEwNjM4Njc2NjgsLT
YzNjkxMDExNiwzNjgzNTYyNjgsMTk5NzMyMzI1NCw4MDgxMjYw
NjAsLTE0MDg0ODUzNDgsLTE0NTUwMzc4NTQsLTE5MjkwOTMwMi
wyNTA4NDkyNTQsLTgxOTM5NDI5NF19
-->