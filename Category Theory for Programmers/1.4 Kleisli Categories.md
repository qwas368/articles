...
您可能已經猜到我並沒有當場發明這個類別。這是所謂的Kleisli範疇（Kleisli Category）的例子 — 一種基於monad的型別。我們尚未準備好討論monad，但我想讓你先了解它們的功能。出於有限的目的，Kleisli範疇有底層程式語言的型別作為物件。從型別A到型別B的態射是使用特定修飾從A到從B派生的類型的函數。
<!--stackedit_data:
eyJoaXN0b3J5IjpbLTE1MzAyNzAzNDksLTEwNjM4Njc2NjgsLT
